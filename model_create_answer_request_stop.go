...
func (dst *CreateAnswerRequestStop) UnmarshalJSON(data []byte) error {
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("data matches more than one schema in oneOf(CreateAnswerRequestStop)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("data failed to match schemas in oneOf(CreateAnswerRequestStop)")
	}
}
...
func NewNullableCreateAnswerRequestStop(val *CreateAnswerRequestStop) *NullableCreateAnswerRequestStop {
	return &NullableCreateAnswerRequestStop{value: val, isSet: true}
}
